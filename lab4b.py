...
def join_lists(l1, l2):
    ...
    return list(set(l1) | set(l2))  # Union of sets, converted back to list without sorting

def match_lists(l1, l2):
    # match_lists will return a list that contains all values found in both l1 and l2
    return sorted(set(l1) & set(l2))  # Intersection of sets, converted back to sorted list

def diff_lists(l1, l2):
    # diff_lists will return a list that contains all different values, which are not shared between the lists
    return list(set(l1) ^ set(l2))  # Symmetric difference of sets, converted back to list without sorting
